#include "MKL25Z4.h"
#include <stdint.h>
#include <stdio.h> // Necesara pentru sprintf
#include "board.h"
...
/* ================= UART2 SERVER (MODIFICAT) ================= */
/* AICI ESTE FIX-UL PENTRU LOGURI */
static void UART2_Init_Send(void) {
    ...
    // Baud Rate 9600
    uint32_t bus_clock = CLOCK_GetBusClkFreq();
    uint32_t divisor = bus_clock / (16 * 9600);

    UART2->BDH = (divisor >> 8) & UART_BDH_SBR_MASK;
    UART2->BDL = (divisor & UART_BDL_SBR_MASK);

    UART2->C2 |= UART_C2_TE_MASK;
}

static void Send_Log(char *msg) {
    while(*msg) {
...
// --- FIX: Trimite HEX manual ca sa nu apara caractere ciudate ---
// Aceasta functie transforma numarul (ex: 0xA4) in text ("A4")
static void Send_Hex_Byte(uint8_t byte) {
    char hexMap[] = "0123456789ABCDEF";

    // Trimitem prima cifra (High Nibble)
    while(!(UART2->S1 & UART_S1_TDRE_MASK));
    UART2->D = hexMap[(byte >> 4) & 0x0F];

    // Trimitem a doua cifra (Low Nibble)
    while(!(UART2->S1 & UART_S1_TDRE_MASK));
    UART2->D = hexMap[byte & 0x0F];
}
...
{
    // dacă încă suntem în fereastra de verde => ON (Deschis)
    if ((int32_t)(msTicks - led_green_until) < 0) {
        Magnet_On();
    } else {
        Magnet_Off();
...
// === FUNCTIE NOUA: CITESTE UID-UL CARDULUI ===
static uint8_t RC522_Anticoll(uint8_t *uid) {
    ...
    RC522_WriteReg(BitFramingReg, 0x80);

    for(int i=0; i<2000; i++) {
        if(RC522_ReadReg(ComIrqReg) & 0x20) {
            // Am primit date, citim UID
            for(int j=0; j<5; j++) uid[j] = RC522_ReadReg(FIFODataReg);
            return 1;
        }
    }
    return 0;
...
static void I2C1_Init(void) {
    ...
    PORTC->PCR[10] = PORT_PCR_MUX(2) | (1u << 5);
...
static void PIN_ProcessKey(char ch, char *buf, uint8_t *idx)
{
    ...
    if (ch == ENTER_KEY) {
        int ok = 0;
        char logBuffer[32]; // Buffer pentru mesajul UART
        ...
        if (ok) {
            ...
            // LOG SUCCES
            sprintf(logBuffer, "PIN: %s [OK]", buf);
            Send_Log(logBuffer);
        } else {
            ...
            // LOG FAIL
            sprintf(logBuffer, "PIN: %s [FAIL]", buf);
            Send_Log(logBuffer);
        }
...
int main(void) {
    ...
    uint8_t atqa[2];
    uint8_t uid[5]; // Pentru ID-ul cardului
    char pinbuf[5] = {0};
    uint8_t pinidx = 0;
    ...
    while (1) {
        ...
        // --- MODIFICARE AICI: CITIRE UID + LOG + USA ---
        if (RC522_Request(atqa)) {
            // Daca detectam card, incercam sa citim si ID-ul
            if(RC522_Anticoll(uid)) {
                // 1. Deschide usa (Magnet + LED)
                LED_Green_3s();

                // 2. Afiseaza pe OLED
                OLED_ShowMessageTimed("CARD ACCES", 2000);

                // 3. Trimite LOG detaliat catre Arduino (MANUAL HEX)
                Send_Log("CARD:");
                Send_Hex_Byte(uid[0]);
                Send_Hex_Byte(uid[1]);
                Send_Hex_Byte(uid[2]);
                ...
                // 4. Pauza mica sa nu citeasca de 100 de ori
                delay_ms(1000);
                RC522_Init(); // Re-initializare necesara
            }
        }

        // --- UPDATE PENTRU ACTUATORI ---
        LED_Update();
        Magnet_Update();
        OLED_Update();
        ...
        if (Button_Pressed()) {
            ...
            // Optional: Log pentru buton
            Send_Log("BUTON: EXIT [OK]\n");

            while (Button_Pressed()) {} // așteaptă eliberarea
        }
